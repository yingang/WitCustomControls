 ...
 *
*/
// Declare dependencies on ControlRegistration, WorkItemTracking/Scripts/Controls/WorkItemForm/WorkItemControl, VSS.Core, VSS.UI modules and UI Widgets.
define(["require", "exports", "WorkItemTracking/Scripts/ControlRegistration", "WorkItemTracking/Scripts/Controls/WorkItemForm/WorkItemControl", "VSS/Utils/Core", "VSS/Utils/UI", "Presentation/Scripts/TFS/TFS.UI.Widgets"], 
    function (require, exports, WITCONTROLREGISTRATION, WITCONTROL, VSSCore, VSSUI) {

        // module content
        var delegate = VSSCore.delegate,
            moduleBaseUrl = TFS.getModuleBase("CodePlex.WitCustomControls.MultiValueControl.js"),
            domElem = VSSUI.domElem;
        ...
        MultiValueControl.inherit(WITCONTROL.WorkItemControl, {
            _control: null,
            _extendedAllowedValues: null,
            _sortArray: [],
            _behavior: null,
            _loadedAllowedValues: false,
            ...
            _init: function () {
                ...
                this._control = $("<div id='" + myId + "_div' style='display: none;'><select id='" + myId + "' multiple='multiple'></select></div><div id='" + myId + "_ac' style='display:none;'></div>").appendTo(this._container);

                $(document).ready(function () {
                    $("#" + myId, that._container).multiselect({
                        noneSelectedText: '...',
                        selectedList: 2,
                        show: "swing",
                        hide: "swing",
                    ...
                    $("#" + myId + "_div", that._container).hide();

                    $("#" + myId + "_ac", that._container).Dropdown({
                        filterList: true,
                    ...
                    $("#" + myId + "_ac", that._container).hide();

                    $("#" + myId + "_ac .dropdown-input-control", that._container).addClass('combo');

                });

            },

            _render: function (workitem, renderExtendedAllowedValues) {
                if (!this._loadedAllowedValues) {
                    var myId = this._options.controlId;
                    ...
                    try {
                        ...
                        // Not supported in new form.
                        console.log("Multi value control not supported in new workitem form.");
                    }

                    if (this._behavior === "autocomplete") {
                        $("#" + myId + "_ac", this._container).show();
                    } else {
                        $("#" + myId + "_div", this._container).show();
                    }

                    //get the allowed values and any remote data returned by the DataProvider
                    var allowedValues = !renderExtendedAllowedValues ? currentText.split(";") : this._getExtendedAllowedValues(field);

                    var mySelect = $("#" + myId, this._container);
                    mySelect.empty();

                    var jqDropdown = $("#" + myId + "_ac", this._container);
                    var dropDownObject = jqDropdown.data('Dropdown') || jqDropdown.data('TFS-Dropdown');
                    dropDownObject.clear();
                    $.each(allowedValues, function (index, val) {
                        ...
                        dropDownObject._addItem({ data: itemToAdd, text: itemToAdd, value: itemToAdd });

                    });

                    if (this._behavior === "autocomplete" && currentText !== "") {
                        var that = this;
                    ...
                    jqDropdown.bind('itemSelected', delegate(this, this._onItemSelected));
                    mySelect.multiselect("refresh");
                    this._control.bind("change", delegate(this, this._onChange));

                    this._loadedAllowedValues = renderExtendedAllowedValues;
                }
            },

            beforeOpen: function () {
                this._render(this._workItem, true);
            },

            bind: function (workitem) {
                this._base(workitem);
                this._render(workitem, false);
            },

            unbind: function () {
                this._loadedAllowedValues = false;
                var myId = this._options.controlId;
                var mySelect = $("#" + myId, this._container);
                this._control.unbind("change", delegate(this, this._onChange));
                mySelect.empty();
                var jqDropdown = $("#" + myId + "_ac", this._container);
                jqDropdown.unbind('itemSelected', delegate(this, this._onItemSelected));
                this._base();
            },
            ...
            _onChange: function () {
                ...
                var allValues = $("#" + myId + " :selected", this._container).map(function () { return $(this).val(); }).get();
            ...
            _onItemSelected: function (event, params) {
                ...
                var jqDropdown = $("#" + this._options.controlId + "_ac", this._container);
                var dropDownObject = jqDropdown.data('Dropdown') || jqDropdown.data('TFS-Dropdown');

                if (this._getField().getValue().indexOf("[" + params.item.text + "]") === -1) {
                    this._addSelected(params.item.text);
            ...
            _removeIdentity: function (parent) {
            ...
            _getExtendedAllowedValues: function (field) {
                ...
                var extendedAllowedValues = field.getAllowedValues();
                ...
                try {
                        ...
                        remoteValues = $.ajax(
                            {
                                ...
                                success: function (data) {
                                    $.each(data, function (index, val) {
                                        ...
                                        if ($.inArray(val, extendedAllowedValues) === -1) {
                                            extendedAllowedValues.push(val);
                                        }
                                    });
                    ...
                    // Not supported for new form.
                    console.log("Multi value control not supported in new workitem form.");
                }

                extendedAllowedValues.sort(function (a, b) {
        ...
        WITCONTROLREGISTRATION.registerWorkItemControl("MultiValueControl", MultiValueControl);
...
(function ($, undefined) {
    ...
    $.widget("ech.multiselect", {
        ...
        _setButtonWidth: function () {
            ...
            // set widths
            this.button.css({ width: '100%' }); // width );
        },
